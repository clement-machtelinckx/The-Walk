...
describe("ResponseService", () => {
    ...
    type SessionResponse = Awaited<ReturnType<typeof SessionRepository.upsertResponse>>;
    type SessionResponses = Awaited<ReturnType<typeof SessionRepository.listResponses>>;
    type TableMemberships = Awaited<ReturnType<typeof MembershipRepository.listByTable>>;
    ...
    describe("respondToSession", () => {
        it("should save response if user is a member", async () => {
            ...
            vi.mocked(SessionRepository.upsertResponse).mockResolvedValue({
                status: "going",
            } as SessionResponse);

            await ResponseService.respondToSession(mockUserId, mockSessionId, { status: "going" });

            expect(SessionRepository.upsertResponse).toHaveBeenCalledWith(
                mockSessionId,
                mockUserId,
                { status: "going" },
            );
        ...
        it("should throw ForbiddenError if user is not a member", async () => {
            ...
            await expect(
                ResponseService.respondToSession(mockUserId, mockSessionId, { status: "going" }),
