...
vi.mock("./session-card", () => ({
    SessionCard: ({ onEdit, canEdit }: SessionCardMockProps) => (
        <div data-testid="session-card">{canEdit && <button onClick={onEdit}>Edit</button>}</div>
    ),
}));
vi.mock("./session-form", () => ({
    SessionForm: ({ onCancel }: SessionFormMockProps) => (
...
vi.mock("./next-session-empty-state", () => ({
    NextSessionEmptyState: ({ onCreate, canCreate }: EmptyStateMockProps) => (
